...
Como administrador, quiero crear departamentos con clave y nombre para organizar a los empleados por area.
...
1. **Given** que no existe un departamento con la clave enviada, **When** el administrador registra un departamento con clave y nombre validos, **Then** el sistema crea el departamento y devuelve sus datos guardados.
2. **Given** que ya existe un departamento con la misma clave, **When** el administrador intenta registrarlo nuevamente, **Then** el sistema rechaza la operacion e informa que la clave debe ser unica.

---

### User Story 2 - Consultar y listar departamentos (Priority: P2)

Como administrador, quiero consultar un departamento por clave y listar departamentos para ubicar rapidamente la informacion y su relacion con empleados.
...
1. **Given** que existe un departamento registrado, **When** el administrador lo consulta por clave, **Then** el sistema devuelve su clave, nombre y la referencia de empleados asociados.
2. **Given** que existen multiples departamentos registrados, **When** el administrador solicita el listado de departamentos, **Then** el sistema devuelve todos los departamentos disponibles con su informacion principal.
3. **Given** que no existe un departamento con la clave consultada, **When** el administrador realiza la consulta, **Then** el sistema responde que el departamento no fue encontrado.
...
Como administrador, quiero actualizar el nombre de un departamento y eliminar departamentos sin empleados asociados, para mantener la estructura organizacional actualizada.
...
1. **Given** que existe un departamento, **When** el administrador actualiza su nombre con un valor valido, **Then** el sistema guarda los cambios y devuelve la version actualizada.
2. **Given** que un departamento tiene empleados asociados, **When** el administrador intenta eliminarlo, **Then** el sistema rechaza la eliminacion e informa que primero deben desvincularse o reasignarse los empleados.
3. **Given** que un departamento no tiene empleados asociados, **When** el administrador lo elimina, **Then** el sistema elimina el registro correctamente.
4. **Given** el departamento tecnico `SIN_DEPTO`, **When** el administrador intenta eliminarlo, **Then** el sistema rechaza la operacion aunque no tenga empleados asociados.
...
- **FR-004**: El sistema MUST permitir listar departamentos con paginacion usando parametros `page` y `size`, con los mismos valores predeterminados que el listado de empleados, devolviendo `employeeCount` por departamento.
